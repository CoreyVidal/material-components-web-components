...
suite('mwc-select:', () => {
  ...
  suite('selection', () => {
    ...
    test('selection via index', async () => {
      assert.isTrue(changeCalls === 1, 'change evt called once on startup');
      changeCalls = 0;
      assert.isTrue(element.value === '', 'initial value is blank');
      assert.isTrue(
      ...
      element.select(1);
      await element.updateComplete;
      assert.isTrue(changeCalls === 1, 'change event called once on selection');
      changeCalls = 0;

      assert.isTrue(element.value === 'a', 'select method updates value');
      assert.isTrue(
